...
public class SnowflakeZookeeperHolder {
    ...
    public boolean init() {
        try {
            ...
            // 存在的话，说明不是第一次启动leaf应用，zk存在以前的【自身节点标识和时间数据】
            else {
                // 自身节点ip:port->0000001
    ...
    private void doService(CuratorFramework curator) {
        // /snowflake/${leaf.name}/forever/ip:port-000000001
        ScheduledUploadData(curator, zk_AddressNode);
    }
    ...
    private void updateLocalWorkerID(int workerID) {
        ...
        LOGGER.info("{} file exists status is {}", LeafconfFile.getAbsolutePath(), exists);
